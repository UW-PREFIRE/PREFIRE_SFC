function Jacobian = JacobianPCRTM(pcrtmpath,ancpath,valid_rad,valid_emis,x, ...
...
%% Jacobian for surface emissivity

molindx = [2 2 2 2 2 2];  % H2O CO2 O3 N2O CO CH4
scalfac = [1.0, 1.0135, 1.0, 1.027, 1.0, 1.145];  % H2O CO2 O3 N2O CO CH4
calc_some_Jacobians = true;
calc_these_Jacobians = [0 0 0 0 0 0 0 0 1];  % H2O CO2 O3 N2O CO CH4 T Ts emis
enable_Jacobian_chan = true;
...
for ibox=1:nboxes
   ...
   for icol=1:length(profiles_for_RTM(ibox).col)
      ...
      % clear sky spectra
      [wv_center, rad_out] = Jacobian_PREFIRE_for_Ts_emis_SRFMar28(wn, ...
                                                    tmprad, SRF, footprint_id);
      Jacobian_for_emis(:,ibox) = rad_out;

      Jacob_emis = zeros(length(valid_rad),length(emis0));
      for j=1:length(valid_emis)
         for i=1:length(valid_rad)
            if valid_rad(i) == valid_emis(j)
               Jacob_emis(i,j) = Jacobian_for_emis(valid_rad(i),ibox);
            end
         end
      end
